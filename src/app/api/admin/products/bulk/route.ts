...
export async function POST(request: Request) {
  ...
  const { productIds, action, categoryId, basePrice, compareAtPrice } = parsed.data;
      ...
      : action === "archive" ? { isActive: false, archivedAt: new Date() }
        : action === "restore" ? { isActive: true, archivedAt: null }
          : action === "category" ? { categoryId: categoryId! }
            : { basePrice: basePrice!, compareAtPrice: compareAtPrice ?? null };
  try {
    const result = await db.$transaction(async (tx) => {
      const updated = await tx.product.updateMany({ where: { id: { in: productIds } }, data });
      if (action === "price") {
