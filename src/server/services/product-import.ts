...
const aliases: Partial<Record<Field, string[]>> = { handle: ["slug", "product_handle", "product slug"], variant_sku: ["sku", "variant sku"], category_slug: ["category", "category slug"], name: ["product_name", "product name", "title", "title_en"], name_ar: ["product_name_ar", "product name ar", "title_ar"], description: ["description_en"], description_ar: ["description_ar"], base_price: ["price", "regular_price", "regular price", "price_kwd"], compare_at_price: ["compare_at_price_kwd"], primary_image_path: ["image", "image_path", "image path"], image_urls: ["image_url", "image url", "remote image url"] };
...
export function previewImport(input: { csv: string; mapping?: Record<string, string>; mode?: ImportMode; imagePathsByRow?: Record<number, string[]> } & ImportContext) {
  ...
  rows.forEach((cells, index) => {
    ...
    const imagePaths = input.imagePathsByRow?.[rowNumber] ?? []; const primaryImagePath = imagePaths[0] ?? nullable(value(record, "primary_image_path", mapping)); const imageUrls = value(record, "image_urls", mapping).split(";").map((url) => url.trim()).filter(Boolean);
