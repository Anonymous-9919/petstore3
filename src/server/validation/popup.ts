...
export const popupInputSchema = z.object({
  ...
  body: nullableText(1000), bodyAr: nullableText(1000), imagePath: nullableText(2000), ctaLabel: nullableText(100), ctaLabelAr: nullableText(100),
  ctaUrl: nullableText(2000), couponCode: nullableText(100).transform((value) => value?.toUpperCase() ?? null), status: z.enum(["DRAFT", "ACTIVE", "ARCHIVED"]).default("DRAFT"),
  ...
  device: z.enum(["ALL", "DESKTOP", "MOBILE"]).default("ALL"), frequency: z.enum(["EVERY_VISIT", "ONCE_PER_SESSION", "ONCE_PER_DAY", "ONCE_PER_X_DAYS"]).default("ONCE_PER_SESSION"), frequencyDays: z.coerce.number().int().min(1).max(365).nullable().optional(), startsAt: nullableDate, endsAt: nullableDate,
